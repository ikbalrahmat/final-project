...
# def analyze_median_imputation(filepath, app, target_column):
...
#     # Imputasi data yang hilang menggunakan Median
#     imputer = SimpleImputer(strategy='median')
#     data_imputed = imputer.fit_transform(X_missing)

#     # Menghitung RMSE untuk hasil imputasi
#     numerical_cols = X.select_dtypes(include=[np.number]).columns
#     rmse = np.sqrt(mean_squared_error(X[numerical_cols], data_imputed[:, :len(numerical_cols)]))

#     # Simpan dataset yang sudah diimputasi
#     df_imputed = pd.DataFrame(data_imputed, columns=X.columns)
#     df_imputed[target_column] = y.values
#     imputed_csv_path = os.path.join(app.config['STATIC_UPLOAD_FOLDER'], 'median_imputed.csv')
#     df_imputed.to_csv(imputed_csv_path, index=False)

#     return {
#         'rmse': rmse,
#         'imputed_csv_path': url_for('static', filename='uploads/median_imputed.csv'),
...
# def analyze_median_imputation(filepath, app, target_column, final_imputation=False):
#     df = pd.read_csv(filepath)
...
#         imputed_csv_path = os.path.join(app.config['STATIC_UPLOAD_FOLDER'], 'median_imputed.csv')
#         df_imputed.to_csv(imputed_csv_path, index=False)
#         imputed_excel_path = os.path.join(app.config['STATIC_UPLOAD_FOLDER'], 'median_imputed.xlsx')
#         df_imputed.to_excel(imputed_excel_path, index=False)
#     else:
#         # Imputasi data asli yang hilang menggunakan Median
...
#         imputed_csv_path = os.path.join(app.config['STATIC_UPLOAD_FOLDER'], 'final_median_imputed.csv')
#         df_imputed.to_csv(imputed_csv_path, index=False)
#         imputed_excel_path = os.path.join(app.config['STATIC_UPLOAD_FOLDER'], 'final_median_imputed.xlsx')
#         df_imputed.to_excel(imputed_excel_path, index=False)

#     return {
#         'rmse': rmse,
#         'method': 'Median Imputation',
#         'imputed_csv_path': url_for('static', filename='uploads/final_median_imputed.csv' if final_imputation else 'uploads/median_imputed.csv'),
#         'imputed_excel_path': url_for('static', filename='uploads/final_median_imputed.xlsx' if final_imputation else 'uploads/median_imputed.xlsx'),
#         'imputed_data': df_imputed.to_html(classes="table table-striped", index=False)
#     }
